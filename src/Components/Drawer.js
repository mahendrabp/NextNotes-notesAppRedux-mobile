...
import {getNotes, getSearchNotes} from '../redux/action/notes';
import {
  addCategory,
  getCategory,
  removeCategory,
  ...
  navLink(nav, text, path, key) {
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          this.props.dispatch(getNotes('', 1, this.props.notes.sorted, text))
        }
        style={styles.FacebookStyle}
        activeOpacity={0.5}>
        <Image source={{uri: path}} style={styles.ImageIconStyle} />
        ...
        <View style={styles.container}>
          ...
          <View style={styles.bottomLink}>
            {this.props.category.category.map((item, index) => {
              ...
              return this.navLink('', item.name, item.image, item.id);
            })}
            {/* <ModalView /> */}
          </View>
        </View>
